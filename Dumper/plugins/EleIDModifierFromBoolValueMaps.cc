...
class EleIDModifierFromBoolValueMaps : public ModifyObjectValueBase
{
public:
	typedef bool valType_t;
	typedef edm::ValueMap<valType_t> valueMap_t;
	typedef edm::EDGetTokenT<valueMap_t> ValMapFloatToken;
...
private:
  ...
  std::unordered_map<unsigned, edm::Handle<valueMap_t > > ele_vmaps;
	...
	if( conf.exists("electron_config") ) {
		...
		for( const std::string& name : parameters ) {
			...
			if( electrons.existsAs<edm::InputTag>(name) ) {
				...
				e_conf.tok_valuemaps[name] = cc.consumes<edm::ValueMap<bool>>(electrons.getParameter<edm::InputTag>(name));
			}
		}
		//e_conf.tok_electron_src = cc.consumes<edm::View<pat::Electron> >(e_conf.electron_src);
	}
...
void EleIDModifierFromBoolValueMaps::setEventContent(const edm::EventSetup& evs)
{
}
