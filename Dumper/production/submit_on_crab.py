...
config.General.workArea = 'LowPt_ScaleSmear_{:s}'.format(production_tag)

config.section_('Data')
config.Data.publication = False
config.Data.outLFNDirBase = '/store/user/ratramon/EGamma/LowPt_SS'
config.Data.inputDBS = 'global'

config.section_('JobType')
config.JobType.pluginName = 'Analysis'
config.JobType.psetName = '../crab/JPsiToEE_dumper_cfg.py'
    ...
    for sample, info in doc['samples'].items():
      ...
      for part in parts:
       ...
       #     'lhcRun={:.0f}'.format(args.lhcRun),
        ...
        config.JobType.outputFiles = ['_'.join(['LowPt_ScaleSmear', 'mc' if isMC else 'data', production_tag])+'.root']
